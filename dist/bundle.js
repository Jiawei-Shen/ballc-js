...
var filehandle = require('generic-filehandle');
...
class BAllC {
    ...
    async getHeader() {
        const headerBuff = buffer.Buffer.allocUnsafe(MADEUP_HEADER_SIZE);
        await this.ballcFileHandle.read(headerBuff, 0, MADEUP_HEADER_SIZE, 0);
        const ungzipedHeader = await bgzfFilehandle.unzip(headerBuff);
    ...
    async getIndexData() {
        ...
        const bytes = await bgzfFilehandle.unzip(buf);
...
async function queryChunk(fileHandle, blockAddress, startOffset, endOffset) {
    ...
    const chunkBuf = buffer.Buffer.allocUnsafe(endOffset);
    await fileHandle.read(chunkBuf, 0, endOffset, blockAddress);
    const unzippedChunk = await bgzfFilehandle.unzip(chunkBuf);
